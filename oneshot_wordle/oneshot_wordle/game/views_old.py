...
def wordle(request): 
    ...
    # print(f"Attempts is: {attempts[0].word}")
    ...
    if request.method == 'POST':   
        ...
        if guess_formset.is_valid() & form.is_valid() & alphabet_formset.is_valid() & attempts.exists()==False:
    ...
    elif (attempts.exists() ==True) & (attempts[0].word == todaysGuessle.word):
        row = guess_result(attempts[0].guess, TARGET_WORD)     
        cluesRow.append(row)
        ...
        guess_form = guess_formset[attempt_number-1]
    elif (attempts ==True) & (attempts[0].word != todaysGuessle.word):
        row = guess_result(attempts[0].guess, TARGET_WORD)     
        cluesRow.append(row)
        messages.add_message(request=request, level=messages.ERROR, message="You have already attempted today's Guessle")
        context['cluesRow'] = cluesRow
